...
const Bar = ({ fastData, longestFastDuration }) => {
  ...
  return (
    <div key={fastData._id} style={{ width: 90, minWidth: 90 }} className="h-full border-l border-r border-gray-200 flex flex-col justify-end items-center">
      <div style={{ height: `calc(${difference / longestFastDuration * 100}% - 16px)` }} className="w-3 bg-green-400 rounded-full" />
      <span className="text-xs font-light text-gray-500">{dayjs(fastData.startedAt).format('MMM D')}</span>
    </div>
  );
};
